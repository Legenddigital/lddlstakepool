...
const (
	...
	defaultConfigFilename   = "lddlstakepool.conf"
	defaultDataDirname      = "data"
	defaultLogLevel         = "info"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "lddlstakepool.log"
	...
	defaultPoolFees         = 7.5
	defaultPoolLink         = "https://forum.Legenddigital.org/threads/rfp-6-setup-and-operate-10-stake-pools.1361/"
...
var (
	lddlstakepoolHomeDir = lddlutil.AppDataDir("lddlstakepool", false)
	defaultConfigFile   = filepath.Join(lddlstakepoolHomeDir, defaultConfigFilename)
	defaultDataDir      = filepath.Join(lddlstakepoolHomeDir, defaultDataDirname)
	defaultLogDir       = filepath.Join(lddlstakepoolHomeDir, defaultLogDirname)
)
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(lddlstakepoolHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	funcName := "loadConfig"
	err = os.MkdirAll(lddlstakepoolHomeDir, 0700)
	if err != nil {
	...
	for idx := range cfg.WalletCerts {
		if !fileExists(cfg.WalletCerts[idx]) {
			path := filepath.Join(lddlstakepoolHomeDir, cfg.WalletCerts[idx])
			if !fileExists(path) {
	...
	if cfg.EnableStakepoold {
		...
		for idx := range cfg.StakepooldCerts {
			if !fileExists(cfg.StakepooldCerts[idx]) {
				path := filepath.Join(lddlstakepoolHomeDir,
