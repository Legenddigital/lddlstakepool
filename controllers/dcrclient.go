...
import (
	...
	"github.com/Legenddigital/lddlstakepool/models"
...
type validateAddressResponse struct {
	addrInfo *lddljson.ValidateAddressWalletResult
	err      error
}

// validateAddressMsg
type validateAddressMsg struct {
	address lddlutil.Address
	reply   chan validateAddressResponse
}

// createMultisigResponse
type createMultisigResponse struct {
	multisigInfo *lddljson.CreateMultiSigResult
	err          error
}
...
type ticketsForAddressResponse struct {
	tickets *lddljson.TicketsForAddressResult
	err     error
}
...
type getStakeInfoResponse struct {
	stakeInfo *lddljson.GetStakeInfoResult
	err       error
}
...
type connectedResponse struct {
	walletInfo []*lddljson.WalletInfoResult
	err        error
}
...
type stakePoolUserInfoResponse struct {
	userInfo *lddljson.StakePoolUserInfoResult
	err      error
}

// stakePoolUserInfoMsg
type stakePoolUserInfoMsg struct {
	userAddr          lddlutil.Address
...
func (w *walletSvrManager) executeInSequence(fn functionName, msg interface{}) interface{} {
	...
	case validateAddressFn:
		...
		vawrs := make([]*lddljson.ValidateAddressWalletResult, w.serversLen)
		connectCount := 0
	...
	case createMultisigFn:
		...
		cmsrs := make([]*lddljson.CreateMultiSigResult, w.serversLen)
		connectCount := 0
		for i, s := range w.servers {
			if w.servers[i] == nil {
				continue
	...
	case ticketsForAddressFn:
		...
		tfars := make([]*lddljson.TicketsForAddressResult, w.serversLen)
		connectCount := 0
		for i, s := range w.servers {
			if w.servers[i] == nil {
				continue
	...
	case getTxOutFn:
		...
		gtors := make([]*lddljson.GetTxOutResult, w.serversLen)
		connectCount := 0
		for i, s := range w.servers {
			if w.servers[i] == nil {
				continue
	...
	case getStakeInfoFn:
		...
		gsirs := make([]*lddljson.GetStakeInfoResult, w.serversLen)
		connectCount := 0
		for i, s := range w.servers {
			if w.servers[i] == nil {
				continue
	...
	case connectedFn:
		...
		resp.err = nil
		wirs := make([]*lddljson.WalletInfoResult, w.serversLen)
	...
	case stakePoolUserInfoFn:
		...
		spuirs := make([]*lddljson.StakePoolUserInfoResult, w.serversLen)
...
func (w *walletSvrManager) connected() ([]*lddljson.WalletInfoResult, error) {
...
func (w *walletSvrManager) syncTickets(spuirs []*lddljson.StakePoolUserInfoResult) error {
...
// call is made.
func (w *walletSvrManager) checkForSyncness(spuirs []*lddljson.StakePoolUserInfoResult) bool {
	for i := 0; i < len(spuirs); i++ {
		if spuirs[i] == nil {
			continue
		}
...
func (w *walletSvrManager) ValidateAddress(addr lddlutil.Address) (*lddljson.ValidateAddressWalletResult, error) {
...
// This should return equivalent results from all wallet RPCs. If this
// encounters a failure, it should be considered fatal.
func (w *walletSvrManager) CreateMultisig(nreq int, addrs []lddlutil.Address) (*lddljson.CreateMultiSigResult, error) {
	// Assert that all servers are online.
	_, err := w.connected()
	if err != nil {
		return nil, connectionError(err)
...
func (w *walletSvrManager) TicketsForAddress(address lddlutil.Address) (*lddljson.TicketsForAddressResult, error) {
...
// from this function should NOT cause fatal errors on the web server like the
// other RPC client calls.
func (w *walletSvrManager) GetTxOut(hash *chainhash.Hash, idx uint32) (*lddljson.GetTxOutResult, error) {
...
// from this function should NOT cause fatal errors on the web server like the
// other RPC client calls.
func (w *walletSvrManager) StakePoolUserInfo(userAddr lddlutil.Address, takeFirstResponse bool) (*lddljson.StakePoolUserInfoResult, error) {
...
// from this function should NOT cause fatal errors on the web server like the
// other RPC client calls.
func (w *walletSvrManager) getStakeInfo() (*lddljson.GetStakeInfoResult, error) {
...
type getTicketsCacheData struct {
	res   *lddljson.TicketsForAddressResult
	timer time.Time
}

// NewGetTicketsCacheData is a contructor for getTicketsCacheData that sets the
// last get time to now.
func NewGetTicketsCacheData(tfar *lddljson.TicketsForAddressResult) *getTicketsCacheData {
...
type walletSvrManager struct {
	...
	cachedStakeInfo      *lddljson.GetStakeInfoResult
...
func (w *walletSvrManager) GetUnspentUserTickets(userMultiSigAddress lddlutil.Address) ([]*chainhash.Hash, error) {
	// live tickets only
...
func (w *walletSvrManager) WalletStatus() ([]*lddljson.WalletInfoResult, error) {
	return w.connected()
}
...
// missing.
func (w *walletSvrManager) fetchTransaction(txHash *chainhash.Hash) (*lddlutil.Tx,
	error) {
	var tx *lddlutil.Tx
	var err error
	for i := range w.servers {
...
// newWalletSvrManager returns a new Legenddigital wallet server manager.
