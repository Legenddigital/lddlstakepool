...
[![GoDoc](https://godoc.org/github.com/Legenddigital/dcrstakepool?status.svg)](https://godoc.org/github.com/Legenddigital/dcrstakepool)
[![Build Status](https://travis-ci.org/Legenddigital/dcrstakepool.svg?branch=master)](https://travis-ci.org/Legenddigital/dcrstakepool)
[![Go Report Card](https://goreportcard.com/badge/github.com/Legenddigital/dcrstakepool)](https://goreportcard.com/report/github.com/Legenddigital/dcrstakepool)

dcrstakepool is a web application which coordinates generating 1-of-2 multisig
addresses on a pool of [dcrwallet](https://github.com/Legenddigital/dcrwallet) servers
so users can purchase [proof-of-stake tickets](https://docs.Legenddigital.org/mining/proof-of-stake/)
on the [Legenddigital](https://Legenddigital.org/) network and have the pool of wallet servers
...
- It is highly recommended to use 3 lddld+dcrwallet+stakepoold nodes for
...
- **KNOWN ISSUE** Total tickets count reported by stakepoold may
  ...
  the database can be marked as voted.  This will be resolved by future work. ([#201](https://github.com/Legenddigital/dcrstakepool/issues/201)).
...
- lddld has a new agenda and the vote version in dcrwallet has been
...
- General markup improvements.
  ...
  - Instructions were further clarified and updated to strongly recommend the
    use of Legenddigitaliton/Paymetheus.
  - Fragments of invalid markup were fixed.

## 1.1.1 Upgrade Guide

1) Announce maintenance and shut down dcrstakepool.
2) Perform upgrades on each lddld+dcrwallet+stakepoold voting cluster one at a
   time.
   * Stop stakepoold, dcrwallet, and lddld.
   * Upgrade lddld, dcrwallet to 1.1.0 release binaries or git. If compiling from
   source, Go 1.9 is recommended to pick up improvements to the Golang runtime.
   * Restart lddld, dcrwallet.
   * Upgrade stakepoold.
   * Start stakepoold.
3) Upgrade and start dcrstakepool.  If you are maintaining a fork, note that
   you need to update the lddld/chaincfg dependency to a revision that contains
   the new agenda.
...
```bash
$ git clone https://github.com/Legenddigital/dcrstakepool $GOPATH/src/github.com/Legenddigital/dcrstakepool
$ cd $GOPATH/src/github.com/Legenddigital/dcrstakepool
$ dep ensure
```
...
```bash
$ cd $GOPATH/src/github.com/Legenddigital/dcrstakepool/backend/stakepoold
$ go build
```
...
These instructions assume you are familiar with lddld/dcrwallet.

- Create basic lddld/dcrwallet/dcrctl config files with usernames, passwords, rpclisten, and network set appropriately within them or run example commands with additional flags as necessary

- Build/install lddld and dcrwallet from latest master

- Run lddld instances and let them fully sync
...
If a user pays an incorrect fee you may add their tickets like so (requires lddld running with txindex=1):
...
- irc.freenode.net
- channel #Legenddigital

## Issue Tracker

The [integrated github issue tracker](https://github.com/Legenddigital/dcrstakepool/issues)
