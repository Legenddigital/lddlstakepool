...
import (
	...
	"github.com/Legenddigital/lddlstakepool/backend/stakepoold/rpc/rpcserver"
	"github.com/Legenddigital/lddlstakepool/backend/stakepoold/userdata"
...
func deriveChildAddresses(key *hdkeychain.ExtendedKey, startIndex, count uint32, params *chaincfg.Params) ([]lddlutil.Address, error) {
	addresses := make([]lddlutil.Address, 0, count)
...
func evaluateStakePoolTicket(ctx *appContext, tx *wire.MsgTx, blockHeight int32, poolUser lddlutil.Address) (bool, error) {
	...
	out := lddlutil.Amount(0)
	for i := range tx.TxOut {
		out += lddlutil.Amount(tx.TxOut[i].Value)
	}
	fees := in - out
	...
	if exists {
		...
		feeNeeded := txrules.StakePoolTicketFee(lddlutil.Amount(
...
func runMain() error {
	...
	if err != nil || walletConn == nil {
		log.Infof("Connection to lddlwallet failed: %v", err)
		return err
	}
	log.Infof("Connected to lddlwallet (JSON-RPC API v%s)",
		walletVer.String())
	...
	if err != nil || nodeConn == nil {
		log.Infof("Connection to lddld failed: %v", err)
		return err
	}
	...
	if err != nil {
		log.Errorf("Unable to get current network from lddld: %v", err)
		return err
	}
	...
	for {
		...
		if err != nil {
			log.Errorf("unable to get bestblock from lddld: %v", err)
			return err
		}
	...
	log.Info("subscribed to notifications from lddld")
...
// getticket pulls the transaction information for a ticket from lddlwallet. This is a go routine!
...
func (ctx *appContext) vote(wg *sync.WaitGroup, blockHash *chainhash.Hash, blockHeight int64, w *ticketMetadata) {
	...
	var res *lddljson.GenerateVoteResult
...
func (ctx *appContext) processNewTickets(nt NewTicketsForBlock) {
	...
	for _, n := range newtickets {
		...
		// decode address
		addr, err := lddlutil.DecodeAddress(n.msa)
		if err != nil {
...
func (ctx *appContext) processWinningTickets(wt WinningTicketsForBlock) {
	...
	for _, ticket := range wt.winningTickets {
		...
		if !ok {
			...
			// plucked from lddlwallet walletinfo then just use the
			// default votebits.
