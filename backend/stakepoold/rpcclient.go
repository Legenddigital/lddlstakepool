...
func connectNodeRPC(ctx *appContext, cfg *config) (*rpcclient.Client, semver, error) {
	...
	lddldCert, err := ioutil.ReadFile(cfg.LddldCert)
	if err != nil {
		log.Errorf("Failed to read lddld cert file at %s: %s\n",
			cfg.LddldCert, err.Error())
		return nil, nodeVer, err
	}

	log.Debugf("Attempting to connect to lddld RPC %s as user %s "+
		"using certificate located in %s",
		cfg.LddldHost, cfg.LddldUser, cfg.LddldCert)

	connCfgDaemon := &rpcclient.ConnConfig{
		Host:         cfg.LddldHost,
		Endpoint:     "ws", // websocket
		User:         cfg.LddldUser,
		Pass:         cfg.LddldPassword,
		Certificates: lddldCert,
	}

	ntfnHandlers := getNodeNtfnHandlers(ctx, connCfgDaemon)
	lddldClient, err := rpcclient.New(connCfgDaemon, ntfnHandlers)
	if err != nil {
		log.Errorf("Failed to start lddld RPC client: %s\n", err.Error())
		return nil, nodeVer, err
	}
	...
	nodeVer = semver{lddldVer.Major, lddldVer.Minor, lddldVer.Patch}
...
func connectWalletRPC(cfg *config) (*rpcclient.Client, semver, error) {
	...
	lddlwCert, err := ioutil.ReadFile(cfg.WalletCert)
	if err != nil {
		log.Errorf("Failed to read lddlwallet cert file at %s: %s\n",
			cfg.WalletCert, err.Error())
		return nil, walletVer, err
	}

	log.Infof("Attempting to connect to lddlwallet RPC %s as user %s "+
		"using certificate located in %s",
	...
	connCfgWallet := &rpcclient.ConnConfig{
		...
		Certificates: lddlwCert,
	}

	ntfnHandlers := getWalletNtfnHandlers(cfg)
	lddlwClient, err := rpcclient.New(connCfgWallet, ntfnHandlers)
	if err != nil {
	...
	ver, err := lddlwClient.Version()
	if err != nil {
		log.Error("Unable to get RPC version: ", err)
		return nil, walletVer, fmt.Errorf("Unable to get node RPC version")
	}

	lddlwVer := ver["lddlwalletjsonrpcapi"]
	walletVer = semver{lddlwVer.Major, lddlwVer.Minor, lddlwVer.Patch}
	...
	return lddlwClient, walletVer, nil
}
...
func walletGetTickets(ctx *appContext, currentHeight int64) (map[chainhash.Hash]string, map[chainhash.Hash]string, error) {
	...
	for _, p := range promises {
		...
		for i := range gt.Details {
			...
			addr, err := lddlutil.DecodeAddress(gt.Details[i].Address)
			if err != nil {
