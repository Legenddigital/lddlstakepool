...
var (
	defaultHomeDir     = lddlutil.AppDataDir("stakepoold", false)
...
// config defines the configuration options for lddld.
//
...
type config struct {
	...
	LddldHost         string  `long:"lddldhost" description:"Hostname/IP for lddld server"`
	LddldUser         string  `long:"lddlduser" description:"Username for lddld server"`
	LddldPassword     string  `long:"lddldpassword" description:"Password for lddld server"`
	LddldCert         string  `long:"lddldcert" description:"Certificate path for lddld server"`
...
func loadConfig() (*config, []string, error) {
	...
	if len(cfg.LddldHost) == 0 {
		str := "%s: lddldhost is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}

	if len(cfg.LddldCert) == 0 {
		str := "%s: lddldcert is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}

	if len(cfg.LddldUser) == 0 {
		str := "%s: lddlduser is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}

	if len(cfg.LddldPassword) == 0 {
		str := "%s: lddldpassword is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}
	...
	cfg.LddldHost = normalizeAddress(cfg.LddldHost, activeNetParams.LddldRPCServerPort)
	cfg.WalletHost = normalizeAddress(cfg.WalletHost, activeNetParams.WalletRPCServerPort)

	if !fileExists(cfg.LddldCert) {
		path := filepath.Join(cfg.HomeDir, cfg.LddldCert)
		if !fileExists(path) {
			str := "%s: lddldcert " + cfg.LddldCert + " and " +
				path + " don't exist"
			err := fmt.Errorf(str, funcName)
			fmt.Fprintln(os.Stderr, err)
			return nil, nil, err
